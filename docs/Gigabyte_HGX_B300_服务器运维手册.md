...
- 序列号条码（如 `XXXXXXXXXXXXXX`）
- BMC 密码条码（如 `XXXXXXXXXXX`）
...
```
B300-R02-15U-PDUA    ← 机型-机柜号-U位-PDU路（A路）
B300-R02-15U-PDUB    ← 机型-机柜号-U位-PDU路（B路）
R02-PDUB-5           ← 机柜号-PDU路-序号
```
...
| 标签类型 | 格式示例 | 字段拆解 |
|----------|----------|----------|
| 存储网络标签 | `C-NNN // s110-r02-u15-ST-2` | C-NNN=配线柜端口 / s110=机房编号 / r02=机柜号 / u15=U位 / ST-2=存储端口2 |
| RoCE 网络标签 | `cls01-s110-r01u18-roce-leaf-31-55` | cls01=集群前缀（占位） / s110=机房 / r01u18=交换机机柜 r01 的 U18 位 / roce-leaf-31=第31号leaf交换机 / 55=端口/链路号 |
| 电源标签 | `B300-R02-15U-PDUB` | B300=机型 / R02=机柜号 / 15U=U位 / PDUB=B路供电 |

> 表中机柜号、配线柜端口号等均为占位示例，非现网实际编号。
