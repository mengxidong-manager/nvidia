...
**完整断电冷启动与 HGX 固件重刷均已验证无效。** 经核对固件包 manifest，现有 HGX-B300x8_0006_260610.1.1_custom 包内为 PSID NVD0000000072 指定的 CX8 版本即为 40.48.1132 —— **故障版本就是本平台的出厂固件**，重刷同版本不会改变行为。

现场已无可自行处置的手段。需厂商提供 CX8 组件版本高于 40.48.1132 的 HGX 固件包，并由 NVIDIA 解读 `ext_synd 0x035e` 的具体语义。
...
| HGX 固件包 | HGX-B300x8_0006_260610.1.1_custom（2026-06-10 发布） |
...
| HGX 固件更新 | `H100_tool_V1.0.11.sh <BMC> <user> <pw> update nvfw_HGX-B300x8_0006_260610.1.1_custom-signed.fwpkg 35` | 刷写成功，22 分钟完成，无报错。但该包内 CX8 组件版本即为 40.48.1132，与在用版本相同，属同版本重刷，故障无变化 |
| 完整冷启动 | — | 整机断电，间隔约 1 分钟后重新上电。错误原样复现：ext_synd 32 条、health compromised 16 条、16 端口仍为 Close port |

### HGX 固件包内容

固件包 manifest 显示，CX8 组件的目标 PSID 为 NVD0000000072、APSKU 为 0x72000000，与现场设备精确匹配，但其版本号即为 40.48.1132。因此本次更新对 CX8 而言是同版本重刷，行为不会改变。CX8 的 40.48.1132 与 CX7 的 28.48.1132 共享 48.1132 基线，属同批协调发布。

| 组件 | PSID / SKU | 包内版本 | 与现场对照 |
|---|---|---|---|
| **CX8** | NVD0000000072 | **40.48.1132** | 与在用版本相同 |
| CX7（ERoT） | NVD0000000079 | 28.48.1132 | 与现场 CX7 一致 |
| ERoT 通用 | — | 01.04.0055.0000_n04 | — |
| ERoT HMC | — | B3-2602-05.0 | — |
| ERoT FPGA | — | 1.54 | — |
| ERoT NVSwitch | NVD0000000113 | 35_2014_5112 | — |
| ERoT NVSwitch | NVD0000000119 | 35_2014_5112 | — |
| SMA | NV0004 | 0004.00.0272.0000 | — |
| SMA | NV000B | 0011.00.0281.0000 | — |
| GPU | PG540_SKU216 | 97.10.7E.00.03 | — |
...
### 1. 向厂商申请更新版本的 HGX 固件包

现有 HGX-B300x8_0006_260610.1.1_custom 包内的 CX8 组件版本即为 40.48.1132，与故障版本相同，重刷无意义。需要 CX8 组件版本高于 40.48.1132 的固件包。PSID NVD0000000072 为 OEM 定制标识，固件只能从技嘉或 NVIDIA 获取，不可使用公版镜像。

向技嘉确认两点：一是是否存在比 0006_260610.1.1 更新的 HGX-B300x8 固件包；二是该包内 CX8 组件版本是否高于 40.48.1132。仅整包版本号递增不足以说明问题，须逐组件核对。

### 2. 提交厂商并要求解读错误码

ext_synd 0x035e 的具体语义需 NVIDIA 内部错误码表解读，现场无法自行判定。本报告及诊断包已包含完整证据链，可直接作为工单附件。

提交时明确说明：冷启动与同版本固件重刷均已验证无效，同机 ConnectX-7 加载正常，问题已隔离至 CX8 固件 40.48.1132 本身。

## 需要留意的事项

**40.48.1132 是本平台当前的出厂固件**　该版本并非现场误刷的旧固件，而是 2026-06-10 发布的 HGX-B300x8_0006_260610.1.1_custom 包内为 PSID NVD0000000072 指定的版本。这意味着问题存在于该平台的出厂固件中，不是配置或操作导致，也无法通过重装现有固件解决。
