...
> 适用场景：模块为 800G 单模、2×400G-DR4 拆分、MPO-8 APC 接口、RoCE 组网。
...
| 物料 | 规格 | 说明 |
|---|---|---|
| MPO-8 APC 跳线 | Base-8 单模，**Type-B 反转极性**，两端母头 | 数量按「待测通道数 ÷ 2」准备。Type-A 直通线接上去 Tx 对 Tx，完全不通 |
| MPO-8 APC 自环模块 | Base-8 单模，选带衰减的版本更省事 | 插在光口上完成 Tx 到 Rx 回接，仅单口自环用，备 1~2 个 |
| MPO 内联衰减器 | 5~10 dB，MPO 母对公 | 与跳线数量相同。自环模块若自带衰减可省 |
| MPO 端面清洁笔 | MPO 专用，非 LC 型号 | 并行光 8 芯，任何一芯脏污都会拉高整体误码 |

> **极性错了链路根本起不来。** Type-B 是并行光模块的行业标准：这类模块把发射端排在连接器一侧、接收端排在另一侧，Type-B 的反转正好让两者自动对齐。MPO-8 沿用与 MPO-12 相同的 Method B 标准，反转逻辑适用于全部 8 个位置。订线时务必指明 Type-B，不要拿手头的 Type-A 干线凑合。

> **衰减器建议加。** DR4 是 500 米短距方案，发射功率比 FR4/LR4 低，自环过载风险没那么大，但短跳线几乎没有衰减，仍可能接近接收过载点。接好后用 `mlxlink -m` 确认各通道 Rx 功率落在规格内，顶到上限就加衰减。
...
一根 MPO-8 Type-B 跳线接在同一个模块的两个 MPO 口之间，通道 1 与通道 2 互为对端。一根线覆盖模块光路、两路 DSP、SerDes 和完整数据面，还能跑 RoCE 压带宽，性价比最高。
...
graph LR
  ...
  eth0 <--> ch1
  eth1 <--> ch2
  ch1 --> J["MPO-8 APC Type-B 跳线<br/>串 5–10 dB 衰减器"]
  J --> ch2
```

Type-B 跳线的芯序是反转的，MPO-8 的映射为 1→8、2→7、3→6、4→5，正好把一端的 4 根发射芯落到另一端的 4 根接收芯上。两端分别插入通道 1 和通道 2 即可，不需手动调换。DR4 的 4 个 100G 通道相互独立，Type-B 造成的 lane 顺序反转由以太网 PCS 的 lane marker 自动对齐。
...
一个 MPO 自环模块插在光口上，模块内部把 4 根发射芯接回 4 根接收芯，光信号原路返回。端口在跟自己通信，MAC 和 GID 相同，链路协商与 FEC 无法正常建立，因此**不能跑 RoCE**，也做不了带宽测试。
...
graph LR
  ...
  TX --> ATT["MPO-8 自环模块<br/>内含 5–10 dB 衰减"]
  ATT --> RX
```

用成品 MPO-8 APC 自环模块即可，不需要自己配跳线，也不涉及极性问题。只在怀疑某个具体通道的光收发有问题、又想排除对端干扰时使用。
...
**极性先于一切。** 自环不通，第一件事是确认跳线是不是 Type-B。用可见光笔从 1 号位打光，另一端亮 8 号位才是 Type-B，亮 1 号位是 Type-A。手头分不清就买极性测试仪，比反复试错快。

**MPO-16 与 MPO-8 不通用。** 如果后续换成单口 MPO-16 的 800G DR8 模块，导向键是偏置的，机械上就插不进 MPO-8 适配器，极性要求也变成 Type-C。换模块前先核对数据手册。

**逐通道看眼图。** 并行光有 8 根纤，任何一根脏污或错位都会拉高整体误码，但从 BER 数字上看不出是哪一根。用 `mlxlink -e` 看各 lane 的眼图张开度，明显劣化的那一路就是问题纤。
